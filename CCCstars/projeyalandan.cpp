...
void guess(int guessCode[4])
{
    ...
    int used[9] = {0}; // 1-9 arasýndaki sayýlarýn kullanýlýp kullanýlmadýðýný kontrol etmek için dizi
    for (int i = 0; i < 4; i++)
    {
        do
        {
            scanf("%d", &guessCode[i]);
            if (guessCode[i] < 1 || guessCode[i] > 9 || used[guessCode[i] - 1])
            {
                printf("Invalid input. Please enter a number between 1 and 9 without duplicates.\n");
            }
        } while (guessCode[i] < 1 || guessCode[i] > 9 || used[guessCode[i] - 1]);
        used[guessCode[i] - 1] = 1; // kullanýlan sayýyý iþaretle
    }
}
